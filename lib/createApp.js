...
async function init() {
  ...
  for (const templateName of templates) {
    fse.readJSON(path.join(__dirname, `/templates/${templateName}/template.json`)).then(templateJson => {
      typeChoices.push({
...
function createApp(name, type, manager) {
  ...
  if (name !== '') fse.ensureDir(name);else exit();

  if (type) {
    fse.copy(path.join(__dirname, `/templates/${type}/template`), root).then(() => {
      for (const templateName of templates) {
        fse.readJSON(path.join(__dirname, `/templates/${templateName}/template.json`)).then(templateJson => {
